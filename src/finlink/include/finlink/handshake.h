#ifndef FINLINK_HANDSHAKE_H
#define FINLINK_HANDSHAKE_H

#include <stddef.h>
#include <stdint.h>

/* App-level handshake (hello / hello_ack / session_ready / handshake_error),
 * exchanged as WebSocket text frames (FINLINK_WS_OPCODE_TEXT,
 * finlink/websocket.h) before any Video/Audio/Input binary frame -- see
 ...
 * text frames (via finlink_ws_build_frame / finlink_ws_parse_frame) and
 * passes this module the frame payload, same division of responsibility as
 * finlink/protocol.h and finlink/websocket.h.
 *
 * protocol_version compatibility is exact-match (docs/protocol.md) --
 * callers should compare finlink_hello.protocol_version against their own
 * FINLINK_PROTOCOL_VERSION themselves and refuse to proceed (with a clear,
...
#define FINLINK_PROTOCOL_VERSION 2

#define FINLINK_MAX_SLOTS 4
#define FINLINK_LABEL_LEN 8
#define FINLINK_STREAM_TYPE_LEN 32
#define FINLINK_INPUT_ENCODING_LEN 32
...
typedef enum {
    ...
    FINLINK_HANDSHAKE_ERR = -1 /* malformed JSON, wrong "message" discriminator, or a required field missing */
} finlink_handshake_result;

typedef struct {
    int index;
    char label[FINLINK_LABEL_LEN]; /* e.g. "P1" */
    int occupied;
} finlink_handshake_slot;

typedef struct {
    uint32_t width;
    uint32_t height;
    double fps;
} finlink_handshake_video;

typedef struct {
    uint32_t sample_rate;
    uint8_t channels;
} finlink_handshake_audio;

/* Server -> client, first message on a freshly WS-upgraded connection. */
typedef struct {
    int protocol_version;
    char stream_type[FINLINK_STREAM_TYPE_LEN]; /* "GC_GBA_LINK", "N3DS_BOTTOM_SCREEN", ... */
    finlink_handshake_slot slots[FINLINK_MAX_SLOTS];
    size_t slot_count; /* truncated to FINLINK_MAX_SLOTS if the server somehow sent more */
    finlink_handshake_video video;
    int has_audio; /* 0 for stream types without audio, e.g. N3DS_BOTTOM_SCREEN */
    finlink_handshake_audio audio;
    char input_encoding[FINLINK_INPUT_ENCODING_LEN]; /* "gba_buttons", ... */
} finlink_hello;
...
typedef struct {
    char code[FINLINK_HANDSHAKE_CODE_LEN]; /* "version_mismatch", "slot_unavailable", "malformed_request" */
    char detail[FINLINK_HANDSHAKE_DETAIL_LEN]; /* human-readable, safe to show directly */
} finlink_handshake_error;

typedef enum {
    FINLINK_HS_MSG_UNKNOWN = 0,
 ...
 * finlink_ws_build_handshake_request. */
