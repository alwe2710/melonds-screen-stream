...
static finlink_json_span whole_object(const uint8_t *data, size_t size) {
...
static finlink_handshake_video parse_video(const char *text, size_t obj_start, size_t obj_end) {
    finlink_handshake_video video;
    memset(&video, 0, sizeof(video));
    const finlink_json_span video_span = finlink_json_find_member(text, obj_start, obj_end, "video");
    if (!video_span.found) {
        return video;
    }
    video.width = (uint32_t)finlink_json_get_number(
        text, finlink_json_find_member(text, video_span.start, video_span.end, "width"));
    video.height = (uint32_t)finlink_json_get_number(
        text, finlink_json_find_member(text, video_span.start, video_span.end, "height"));
    video.fps = finlink_json_get_number(
        text, finlink_json_find_member(text, video_span.start, video_span.end, "fps"));
    return video;
}

/* Shared by finlink_parse_hello and finlink_parse_session_ready: both carry
...
static int parse_audio(const char *text, size_t obj_start, size_t obj_end,
                        finlink_handshake_audio *out) {
    const finlink_json_span audio_span = finlink_json_find_member(text, obj_start, obj_end, "audio");
    if (!audio_span.found) {
        memset(out, 0, sizeof(*out));
        return 0;
    }
    out->sample_rate = (uint32_t)finlink_json_get_number(
        text, finlink_json_find_member(text, audio_span.start, audio_span.end, "sample_rate"));
    out->channels = (uint8_t)finlink_json_get_number(
        text, finlink_json_find_member(text, audio_span.start, audio_span.end, "channels"));
    return 1;
}

finlink_handshake_message_type finlink_peek_handshake_message(const uint8_t *data, size_t size) {
    const char *text = (const char *)data;
    const finlink_json_span obj = whole_object(data, size);
    char message[16];
    if (finlink_json_get_string(text, finlink_json_find_member(text, obj.start, obj.end, "message"),
                                 message, sizeof(message)) == (size_t)-1) {
        return FINLINK_HS_MSG_UNKNOWN;
    }
    if (strcmp(message, "hello") == 0) {
        return FINLINK_HS_MSG_HELLO;
    }
    if (strcmp(message, "session_ready") == 0) {
        return FINLINK_HS_MSG_SESSION_READY;
    }
    if (strcmp(message, "handshake_error") == 0) {
        return FINLINK_HS_MSG_HANDSHAKE_ERROR;
    }
    return FINLINK_HS_MSG_UNKNOWN;
}

finlink_handshake_result finlink_parse_hello(const uint8_t *data, size_t size, finlink_hello *out) {
    const char *text = (const char *)data;
    const finlink_json_span obj = whole_object(data, size);

    if (finlink_peek_handshake_message(data, size) != FINLINK_HS_MSG_HELLO) {
        return FINLINK_HANDSHAKE_ERR;
    }
    memset(out, 0, sizeof(*out));
    ...
    const finlink_json_span slots_span = finlink_json_find_member(text, obj.start, obj.end, "slots");
    if (slots_span.found) {
        finlink_json_span elem = {0, 0, 0};
        while (out->slot_count < FINLINK_MAX_SLOTS) {
            elem = finlink_json_array_next(text, slots_span.start, slots_span.end, elem);
            if (!elem.found) {
                break;
            }
            finlink_handshake_slot *slot = &out->slots[out->slot_count];
            slot->index = (int)finlink_json_get_number(
                text, finlink_json_find_member(text, elem.start, elem.end, "index"));
            get_string_field(text, elem.start, elem.end, "label", slot->label, sizeof(slot->label));
            slot->occupied = finlink_json_get_bool(
                text, finlink_json_find_member(text, elem.start, elem.end, "occupied"));
            out->slot_count++;
        }
    }
...
finlink_handshake_result finlink_parse_session_ready(const uint8_t *data, size_t size,
                                                      finlink_session_ready *out) {
    const char *text = (const char *)data;
    const finlink_json_span obj = whole_object(data, size);

    if (finlink_peek_handshake_message(data, size) != FINLINK_HS_MSG_SESSION_READY) {
        return FINLINK_HANDSHAKE_ERR;
    }
    memset(out, 0, sizeof(*out));
    ...
    if (redirect_span.found) {
        ...
        out->redirect_port = (int)finlink_json_get_number(
            text, finlink_json_find_member(text, redirect_span.start, redirect_span.end, "port"));
    }
...
finlink_handshake_result finlink_parse_handshake_error(const uint8_t *data, size_t size,
    ...
    if (finlink_peek_handshake_message(data, size) != FINLINK_HS_MSG_HANDSHAKE_ERROR) {
        return FINLINK_HANDSHAKE_ERR;
    }
    memset(out, 0, sizeof(*out));
