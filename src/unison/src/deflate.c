...
unison_deflate_status unison_deflate_raw(const uint8_t *src, size_t src_size, uint8_t *out_buf,
     ...
     * unison_inflate_raw() (and every client's decoder) expects -- see
     ...
     * real CPU budget to spend, unlike unison_inflate_raw()'s
    ...
    if (written == 0) {
        return UNISON_DEFLATE_ERR;
    }

    *out_size = written;
    return UNISON_DEFLATE_OK;
}
