#ifndef UNISON_PROTOCOL_H
#define UNISON_PROTOCOL_H
...
typedef enum {
    ...
    UNISON_MSG_INPUT = 2,
    UNISON_MSG_AUDIO = 3,
    UNISON_MSG_TEXT_INPUT_REQUEST = 4,  /* server->client, see unison_text_input_request */
...
typedef enum {
    UNISON_OK = 0,
    UNISON_ERR_TOO_SHORT = -1,     /* buffer/decompressed data smaller than expected */
    UNISON_ERR_UNKNOWN_TYPE = -2,  /* leading byte isn't a known unison_msg_type */
    UNISON_ERR_UNKNOWN_FORMAT = -3 /* video_header.format isn't a known unison_video_format */
} unison_result;
...
typedef enum {
    UNISON_KEY_A = 1 << 0,
    UNISON_KEY_B = 1 << 1,
    UNISON_KEY_SELECT = 1 << 2,
    UNISON_KEY_START = 1 << 3,
    UNISON_KEY_RIGHT = 1 << 4,
    UNISON_KEY_LEFT = 1 << 5,
    UNISON_KEY_UP = 1 << 6,
    UNISON_KEY_DOWN = 1 << 7,
    UNISON_KEY_R = 1 << 8,
...
typedef enum {
    ...
    UNISON_VIDEO_FORMAT_INDEXED = 1 << 0,
     ...
     * are -- unison_decode_video_frame() patches just those tiles into
...
typedef struct {
    uint32_t width;
    uint32_t height;
    uint8_t format; /* unison_video_format bits, OR'd together */
 ...
 * unison_msg_type (the receiver already knows which shape to expect from
...
#define UNISON_TOUCH_FRAME_SIZE 6
 ...
 *
 * touch/touch_x/touch_y: same semantics as unison_touch_state (x/y
 * meaningless, sent as 0, whenever pressed is 0).
 *
 * buttons: a generic superset bitmask (unison_button_bit) covering every
...
typedef struct {
    ...
    int16_t right_x;
    int16_t right_y;
} unison_extended_input;

/* unison_extended_input.buttons bit assignments -- a bit only means
 * something to a server whose console actually has that button, see
 * unison_extended_input's own comment. Named after the button each bit
 * most directly corresponds to across consoles (e.g. UNISON_BUTTON_X/Y
...
typedef enum {
    UNISON_BUTTON_A = 1u << 0,
    UNISON_BUTTON_B = 1u << 1,
    UNISON_BUTTON_X = 1u << 2,
    UNISON_BUTTON_Y = 1u << 3,
    UNISON_BUTTON_L = 1u << 4,
    UNISON_BUTTON_R = 1u << 5,
    UNISON_BUTTON_ZL = 1u << 6,
    UNISON_BUTTON_ZR = 1u << 7,
    UNISON_BUTTON_SELECT = 1u << 8, /* aka Minus (Wii U) */
    UNISON_BUTTON_START = 1u << 9,  /* aka Plus (Wii U) */
    UNISON_BUTTON_UP = 1u << 10,
    UNISON_BUTTON_DOWN = 1u << 11,
    UNISON_BUTTON_LEFT = 1u << 12,
    UNISON_BUTTON_RIGHT = 1u << 13,
    UNISON_BUTTON_HOME = 1u << 14,
} unison_button_bit;

#define UNISON_EXTENDED_INPUT_FRAME_SIZE 18

/* Writes out_buf[UNISON_EXTENDED_INPUT_FRAME_SIZE] (the caller must have
 * room for that many bytes). Returns the number of bytes written, always
 * UNISON_EXTENDED_INPUT_FRAME_SIZE -- same convention as
 * unison_build_touch_frame. */
 ...
 * "n3ds_touch_and_buttons" -- see unison_extended_input's own comment. */
 ...
 * Same touch/buttons semantics as unison_extended_input (touch_x/touch_y
 * meaningless when pressed is 0; buttons is the same unison_button_bit
 ...
 * unison_extended_input would otherwise carry on a console with none --
...
typedef struct {
    ...
    uint16_t touch_y;
    uint32_t buttons;
} unison_touch_and_buttons;

#define UNISON_TOUCH_AND_BUTTONS_FRAME_SIZE 10

/* Writes out_buf[UNISON_TOUCH_AND_BUTTONS_FRAME_SIZE] (the caller must
 * have room for that many bytes). Returns the number of bytes written,
 * always UNISON_TOUCH_AND_BUTTONS_FRAME_SIZE -- same convention as
 * unison_build_touch_frame. */
 ...
 * see unison_touch_and_buttons's own comment. */
...
/* Upper bound on unison_inflate_raw()'s output size for a video message of
 * this width/height, across all four unison_video_format combinations --
 * use this to size the buffer passed to unison_inflate_raw(), not
...
/* Decodes the unison_inflate_raw() output of a video message's
 * compressed_data (format-dependent, see unison_video_format) into
 ...
 * width/height every call). If format has UNISON_VIDEO_FORMAT_TILES set,
 * only the pixels belonging to the changed tiles are overwritten; every
 * other pixel keeps whatever unison_decode_video_frame() last wrote
 ...
 * UNISON_INPUT_FRAME_SIZE bytes). Returns the number of bytes written. */
 ...
 * room for UNISON_TOUCH_FRAME_SIZE bytes). Returns the number of bytes
 * written. */
 ...
 * "n3ds_touch" -- see unison_touch_state's own comment. */
...
} unison_text_input_request;

/* Header only (type + max_length + text_len); the text itself follows and
 * is variable-length, see unison_text_input_request_max_size(). */
#define UNISON_TEXT_INPUT_REQUEST_HEADER_SIZE 9

size_t unison_text_input_request_max_size(size_t text_len);

/* Writes into out_buf (sized with unison_text_input_request_max_size()).
...
typedef struct {
    ...
    size_t text_len;   /* in bytes */
} unison_text_input_response;

/* Header only (type + confirmed + text_len); the text itself follows and is
 * variable-length, see unison_text_input_response_max_size(). */
#define UNISON_TEXT_INPUT_RESPONSE_HEADER_SIZE 6

size_t unison_text_input_response_max_size(size_t text_len);

/* Writes into out_buf (sized with unison_text_input_response_max_size()).
 * Returns the number of bytes written, or 0 if out_capacity was too small. */
...
#define UNISON_MIC_ENABLE_FRAME_SIZE 6

/* Writes out_buf[UNISON_MIC_ENABLE_FRAME_SIZE] (the caller must have room
 * for that many bytes). Returns the number of bytes written, always
 * UNISON_MIC_ENABLE_FRAME_SIZE. */
 ...
 * the client's own microphone, sent while a UNISON_MSG_MIC_ENABLE(1) is in
 * effect. Always mono (consoles with a mic input, Wii U GamePad and 3DS,
 * only ever take a single channel). Same wire shape as unison_audio_frame
 ...
 * the layout differs). No build helper here since, like UNISON_MSG_AUDIO,
...
unison_result unison_parse_mic_audio_frame(const uint8_t *data, size_t size,
                                              unison_audio_frame *out);

#ifdef __cplusplus
}
#endif

#endif /* UNISON_PROTOCOL_H */
